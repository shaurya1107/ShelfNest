...
export default function ItemDetail() {
  ...
  const fetchItem = async () => {
    try { setLoading(true); const data = await getItem(id); setItem(data); }
    catch (err) { toast.error('Item not found'); navigate('/'); }
    finally { setLoading(false); }
  };

  useEffect(() => { fetchItem(); }, [id]);

  const handleDateSelect = (start, end) => { setStartDate(start); setEndDate(end); };

  const handleBooking = async () => {
    if (!startDate || !endDate) { toast.error('Please select start and end dates'); return; }
    setSubmitting(true);
    try {
      await createBooking({ item_id: parseInt(id), start_date: format(startDate, 'yyyy-MM-dd'), end_date: format(endDate, 'yyyy-MM-dd'), notes: bookingNotes });
      toast.success('Booking request sent! The owner will review it.');
      setStartDate(null); setEndDate(null); setBookingNotes(''); fetchItem();
  ...
  const handleReview = async (bookingId) => {
    ...
    try {
      await createReview({ booking_id: bookingId, rating: reviewRating, comment: reviewComment });
      toast.success('Review submitted!'); setReviewRating(0); setReviewComment(''); fetchItem();
    } catch (err) { toast.error(err.message); } finally { setReviewSubmitting(false); }
  };

  if (loading) return <div className="page-container"><div className="loading-state"><div className="spinner spinner-lg" /><span>Loading...</span></div></div>;
  if (!item) return null;
      ...
      <div className="detail-layout">
        <div>
          ...
          <div className="detail-image mb-2">
            {item.image_url ? <img src={item.image_url} alt={item.title} /> : (
              <div className="detail-image-placeholder">
                <span className="material-symbols-outlined" style={{ fontSize: 80, color: 'var(--text-tertiary)' }}>inventory_2</span>
              </div>
            )}
          </div>
          ...
          <div className="card">
            ...
            <h1 style={{ fontFamily: 'var(--font-display)', fontSize: '1.5rem', fontWeight: 700, marginBottom: '0.75rem' }}>{item.title}</h1>
            <p style={{ color: 'var(--text-secondary)', lineHeight: 1.7, marginBottom: '1.25rem' }}>{item.description}</p>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
              ...
              {item.deposit_amount > 0 && (
                <div className="flex-gap text-sm">
                  ...
                  <strong style={{ color: 'var(--color-primary)' }}>₹{item.deposit_amount}</strong>
                </div>
              )}
              {item.avg_rating && (
                <div className="flex-gap text-sm">
                  <span className="material-symbols-outlined" style={{ fontSize: 16, color: 'var(--color-primary)', fontVariationSettings: "'FILL' 1" }}>star</span>
          ...
          <div className="card mt-2">
            ...
            {completedBookings.filter(b => !reviewedBookingIds.has(b.id)).map(booking => (
              <div key={booking.id} className="card mt-2" style={{ background: 'var(--color-surface-container)' }}>
                <h3 className="text-sm font-medium mb-1">Leave a Review</h3>
                <div className="mb-1"><StarRating rating={reviewRating} onChange={setReviewRating} interactive size={24} /></div>
                <div className="form-group">
                  <textarea className="form-input" placeholder="Share your experience..." value={reviewComment} onChange={(e) => setReviewComment(e.target.value)} rows={3} style={{ minHeight: 80 }} />
                </div>
                <button className="btn btn-primary btn-sm" onClick={() => handleReview(booking.id)} disabled={reviewSubmitting}>
                  <span className="material-symbols-outlined" style={{ fontSize: 14 }}>send</span> Submit Review
                </button>
              </div>
            ))}
          </div>
          ...
          {!isOwner && item.is_available ? (
            <div className="card">
              <h2 style={{ fontSize: '1.125rem', marginBottom: '1rem' }}>
                <span className="material-symbols-outlined" style={{ fontSize: 20, verticalAlign: 'text-bottom', marginRight: '0.5rem' }}>calendar_month</span>
                Book This Item
              </h2>
              <BookingCalendar bookedRanges={bookedRanges} selectedStart={startDate} selectedEnd={endDate} onSelect={handleDateSelect} />
              {startDate && (
              ...
              <div className="form-group mt-2">
                ...
                <textarea className="form-input" placeholder="Any special instructions..." value={bookingNotes} onChange={(e) => setBookingNotes(e.target.value)} rows={2} style={{ minHeight: 60 }} />
              </div>
              <button className="btn btn-primary btn-lg btn-full" onClick={handleBooking} disabled={!startDate || !endDate || submitting} id="submit-booking">
            ...
            <div className="card">
              ...
              <button className="btn btn-secondary btn-full" onClick={() => navigate(`/items/edit/${id}`)}>
                <span className="material-symbols-outlined" style={{ fontSize: 16 }}>edit</span> Edit Item
              </button>
            </div>
          ) : (
            <div className="card"><p className="text-muted text-sm">This item is currently unavailable for booking.</p></div>
          )}
        </div>
      </div>
    </div>
